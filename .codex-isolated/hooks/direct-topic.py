...
def _save_topic(home: Path, session_id: str, root: Path, topic: str) -> None:
    value = json.dumps(
        {"root": str(root), "topic": topic},
        sort_keys=True,
        separators=(",", ":"),
    ) + "\n"
...
def _load_topic(home: Path, session_id: str, root: Path) -> dict[str, str] | None:
    ...
    if SLUG_RE.fullmatch(value["topic"]) is None:
        return None
    return {"root": value["root"], "topic": value["topic"]}
...
def handle(event: Mapping[str, object], environ: Mapping[str, str]) -> dict[str, object] | None:
    ...
    try:
        ...
        if topic_match is not None:
            ...
            _save_topic(home, session_id, root, topic)
            return _context(
                f"Direct topic {topic} is active. Its explicit @topic command approved "
                "the initial engineering profile."
            )
        current = _load_topic(home, session_id, root)
        if current is None:
            return None
        return _context(f"Direct topic {current['topic']} is active.")
