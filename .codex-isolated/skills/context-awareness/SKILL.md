...
IF MCP-сервер iwiki подключён:
  ...
  3. wiki_status → storage, transport, список `domains`, текущая привязка read/write/primary. On hosted HTTP, trust status only when `binding_source == "session"` and requested primary remains unchanged; otherwise rebind and repeat once, or report `completion-pending`.
