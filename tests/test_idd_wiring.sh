...
assert_eq "opt-out base removes idd-gate" "0" "$(grep -c 'idd-gate.py' <<<"$hooks_off_base")"
assert_eq "opt-out base removes idd-nudge" "0" "$(grep -c 'idd-nudge.py' <<<"$hooks_off_base")"
...
ensure_idd_wiring
count="$(grep -c "idd-gate.py" "$ICODEX_HOME_DIR/hooks.json")"
