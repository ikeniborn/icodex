...
code=0
source_ref_file="$tmp/vendor/superpowers/source-ref"
_vendor_publish "$scratch" "$dest" "$pin" "$patches" "$source_ref_file" "deadbeef" || code=$?
...
assert_eq "source ref published" "deadbeef" "$(cat "$source_ref_file")"
...
_vendor_publish "$scratch" "$dest" "$pin" "$patches" "$source_ref_file" "deadbeef"
...
drift_code=0
_vendor_publish "$scratch" "$dest" "$pin" "$patches" "$source_ref_file" "deadbeef" >/dev/null 2>&1 || drift_code=$?
...
conflict_code=0
_vendor_publish "$scratch" "$dest" "$pin" "$patches" "$source_ref_file" "deadbeef" >/dev/null 2>&1 || conflict_code=$?
...
rm -rf "$scratch"/*
missing_code=0
_vendor_publish "$scratch" "$dest" "$pin" "$patches" "$source_ref_file" "deadbeef" >/dev/null 2>&1 || missing_code=$?
...
ambiguous_code=0
_vendor_publish "$scratch" "$dest" "$pin" "$patches" "$source_ref_file" "deadbeef" >/dev/null 2>&1 || ambiguous_code=$?
...
# Committed pristine upstream plus patches must reproduce materialized skills.
...
cp "$ROOT/vendor/superpowers/upstream/skills/brainstorming/SKILL.md" "$clean/skills/brainstorming/SKILL.md"
cp "$ROOT/vendor/superpowers/upstream/skills/writing-plans/SKILL.md" "$clean/skills/writing-plans/SKILL.md"
assert_eq "committed source ref matches pin generation" "11c74d6b" "$(cat "$ROOT/vendor/superpowers/source-ref")"
...
mandatory_code=0
_vendor_publish "$scratch" "$dest" "$pin" "$patches" "$source_ref_file" "deadbeef" >/dev/null 2>&1 || mandatory_code=$?
...
extra_code=0
_vendor_publish "$scratch" "$dest" "$pin" "$patches" "$source_ref_file" "deadbeef" >/dev/null 2>&1 || extra_code=$?
...
semantic_code=0
_vendor_publish "$scratch" "$dest" "$pin" "$patches" "$source_ref_file" "deadbeef" >/dev/null 2>&1 || semantic_code=$?
...
manifest_code=0
_vendor_publish "$scratch" "$dest" "$pin" "$patches" "$source_ref_file" "deadbeef" >/dev/null 2>&1 || manifest_code=$?
...
run_wrapper_case() { # <mode> <expected-exit>
  ...
  rm -rf "$wrapper_tmp"/*
  output="$(VENDOR_ROOT="$wrapper_root" TMPDIR="$wrapper_tmp" FAKE_CODEX_MODE="$mode" FAKE_UPSTREAM="$ROOT/vendor/superpowers/upstream" \
...
assert_eq "wrapper persists supplied source ref" "deadbeef" "$(cat "$wrapper_root/vendor/superpowers/source-ref")"
