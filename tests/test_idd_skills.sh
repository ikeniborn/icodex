...
if [[ -f "$FI" ]]; then
  ...
  assert_contains "fix-intent creates draft topic profile" "$body" 'Create the draft topic profile before validation'
  assert_before "fix-intent profile precedes intent validation" "$body" 'Create the draft topic profile before validation' 'Run `$check-chain intent'
  assert_contains "fix-intent profile includes selection task" "$body" 'id: intent-profile-selection'
  assert_contains "fix-intent profile selects routed profile" "$body" 'preferred_profiles:'
  assert_contains "fix-intent profile remains unapproved" "$body" 'status: draft'
