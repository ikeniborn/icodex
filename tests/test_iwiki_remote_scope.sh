...
assert_contains "remote scope forwards specification mode" "$agents" 'map `[specifications].mode` to `specification_mode`'
assert_contains "remote scope prefers structured policy" "$agents" '`project_policy`'
assert_contains "remote scope carries snapshot age" "$agents" '`[code_graph].max_snapshot_age_seconds`'
assert_contains "remote scope does not mix aliases" "$agents" 'Never send both'
assert_contains "remote scope preserves full policy on rebind" "$agents" 'replaces the policy wholesale'
assert_contains "remote scope handles tenant overrides" "$agents" 'tenant-wide override'
assert_contains "remote scope reads field suppression" "$agents" '`policy.domains[].suppressed`'
assert_contains "remote scope targets link refresh explicitly" "$agents" '`wiki_code_refresh_links(domain=...)`'
assert_contains "remote session-binding policy is server-owned" "$agents" '`require_session_binding` is operator-only'
assert_contains "remote bind excludes server-owned policy" "$agents" 'never include it in `project_policy`'
assert_eq "remote never maps project session binding" "0" "$(grep -cF '[code_graph].require_session_binding' <<<"$agents")"
...
assert_contains "remote scope states hosted precedence" "$agents" 'exact override, then tenant-wide override, then the carried project value, then hosted default'
...
assert_contains "remote scope repairs default binding and retries read" "$flat_agents" 'If status, a domain-free code read, or a `wiki_spec_search` or `wiki_search` called without `domains` reports `token_default` or `binding_defaulted`, call `wiki_bind` again with the exact project scope and repeat the affected read.'
assert_contains "remote scope names the defaulted write-intent target" "$flat_agents" '`wiki_search(intent="write")` prefers the bound primary over any `domains` argument'
assert_contains "remote scope reads refusal provenance first" "$flat_agents" 'A refused call answers `access_denied` whose `data` carries your own `binding_source` and, when the gate can attribute it, a `reason`'
assert_contains "remote scope names the resolve refusal reasons" "$flat_agents" '`not_bound_primary` reports a scenario outside that primary and is never repaired by requesting a wider grant'
