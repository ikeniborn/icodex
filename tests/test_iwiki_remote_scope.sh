...
assert_contains "remote scope repairs default binding and retries read" "$flat_agents" 'If status, a domain-free code read, or a `wiki_spec_search` called without `domains` reports `token_default` or `binding_defaulted`, call `wiki_bind` again with the exact project scope and repeat the affected read.'
