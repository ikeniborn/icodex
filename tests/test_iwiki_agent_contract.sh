...
assert_contains "rules name the defaulted markdown search scope" "$flat_agents_body" 'without `domains` its search set is the bound read list, and `intent="write"` prefers the bound primary over any `domains` you pass'
...
assert_contains "GWT names the defaulted search scope" "$flat_agents_body" 'A `wiki_spec_search` without `domains` searches the bound read list, so under `token_default` it reports `binding_defaulted`'
assert_contains "GWT resolve targets the bound primary" "$flat_agents_body" 'Hosted `wiki_spec_resolve` accepts only the bound primary and attributes a refusal as `invalid_domain`, `primary_not_selected`, `primary_not_writable`, or `not_bound_primary`'
assert_contains "GWT reads a refused resolve as a binding mismatch" "$flat_agents_body" 'reports a scenario living outside the bound primary, not a missing grant'
...
assert_contains "modes name the defaulted specification search" "$flat_modes_body" 'a `wiki_spec_search` or `wiki_search` without `domains` reports `token_default` or `binding_defaulted`'
