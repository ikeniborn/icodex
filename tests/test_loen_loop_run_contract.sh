...
wrong_even_indent_authority_output="$(PYTHONPATH="$hook_root" python3 - 2>/dev/null <<'PY'
from loen_common import parse_loop_yaml, parse_loop_yaml_checked
...
assert_eq "wrong even indentation fails closed across canonical authorities" "OK" "$wrong_even_indent_authority_output"
