...
В workflow `chain` английские markdown-артефакты остаются source of truth. Ветка `execute` использует `intent -> result`, а ветка `full` — `intent -> spec -> plan -> result`.
...
В ветке `full` `check-chain spec` отдельно проверяет дизайн на ошибки и полноту: что найдено, что исправлено или ещё требует доработки, что будет спроектировано, какие требования будут закрыты, и какие критерии приёмки подтверждают дизайн до перехода к плану.

В ветке `full` `check-chain plan` отдельно проверяет план на ошибки и полноту: что найдено, что исправлено или ещё требует доработки, что будет реализовано, какие проблемы или требования план закрывает, и какими командами/свидетельствами это нужно будет подтвердить после исполнения. Пользователя подключают только на настоящих развилках: спорный scope, противоречивые артефакты или решение, которое нельзя безопасно принять из текущих источников.

Промежуточный HTML для `intent`, `spec` и `plan` не создаётся. Только на `check-chain result` агент спрашивает, нужен ли HTML-отчёт, когда уже есть implementation evidence, diff reconciliation, verification evidence и documentation evidence; если пользователь отказывается, HTML не создаётся и не обновляется.
...
следующего task. Обычная интерактивная работа продолжает выполняться на активной parent
model без обязательных `/model` или `/status`. Для подходящих самостоятельных subtasks
parent явно передаёт child agent выбранные `model` и `reasoning_effort`. Hook только
валидирует orchestrated evidence, никогда не выбирает и не меняет model.
...
> `ICODEX_IWIKI_SYSTEM1_SHADOW`, `ICODEX_IWIKI_SYSTEM1_BASE_URL` и секрет
...
> Когда вендоренный релиз Superpowers переименовывает свой marketplace, wiring при запуске
> автоматически мигрирует старую identity в каждом существующем per-project `config.toml`
> (переименовывается только единственная однозначная пара таблиц marketplace/plugin
> superpowers; любая неоднозначность падает fail-closed с `marketplace mismatch`).
...
icodex передаёт `--dangerously-bypass-hook-trust` при запуске, чтобы проверенные встроенные
хуки работали без интерактивного запроса. В текущих версиях Codex постоянного TOML-аналога нет.
